...
export class SchedulesService {
  ...
  // service : done
  async getSchedulePeriodById(periodId: number) {
    try {
  ...
  // service : done
  async getSchedulesByClassroom(@Param('classroomId') classroomId: number) {
    try {
      await this.handleScheduleActivation();
  ...
  // service : done
  async getWeeklyCompleteSchedule(@Param('classroomId') classroomId: number) {
    try {
      ...
      return {
        ...
        message: 'Weekly schedule retrieved successfully',
        metadata: {
          totalSlots: activePeriod.schedules.length,
        },
        weeklySchedule: {
          name: activePeriod.name,
          startDate: activePeriod.startDate,
  ...
  async createSchedulePeriod(
    ...
    try {
      ...
      // scheduled period must have startDate and name
      if (type !== 'current') {
        if (!body.startDate) {
          return {
            success: false,
            message: 'Start date is required for non-current periods',
            error: 'Invalid type for schedule period',
            statusCode: 400,
          };
        }
        if (!body.name) {
          return {
            success: false,
            message: 'Name is required for non-current periods',
            error: 'Invalid type for schedule period',
            statusCode: 400,
          };
        }
      ...
      const schedulePeriod = await this.prismaService.schedulePeriod.create({
        data: {
          ...
          name:
            type === 'current'
              ? `${classroom.name}-${body.startDate}-${body.endDate}`
              : body.name!,
          startDate:
            type === 'current'
              ? new Date(
  ...
  async updateSchedulePeriod(
    ...
    try {
        ...
        data: {
          ...
          isActive:
            ...
            (body.startDate
              ...
              ? true
              : false,
          name: existingPeriod.isActive
            ? `${existingPeriod.name.includes('-') ? existingPeriod.name.split('-')[0] : existingPeriod}
            -${body.startDate || existingPeriod.startDate.toISOString().split('T')[0]}
            /${existingPeriod.endDate ? existingPeriod.endDate.toISOString().split('T')[0] : 'open'}`.replace(
                /\s+/g,
                '',
              )
            : existingPeriod.name,
      ...
      if (!updatedPeriod) {
        return {
          success: false,
          message: 'Failed to update schedule period',
  ...
  async deleteSchedulePeriod(@Param('periodId') periodId: number) {
    try {
      ...
      const deletedPeriod = await this.prismaService.schedulePeriod.delete({
        where: { id: periodId },
      });

      if (!deletedPeriod) {
        return {
          message: 'Failed to delete schedule period',
          success: false,
          statusCode: 500,
        };
      }
